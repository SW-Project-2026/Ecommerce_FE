...
export default function CheckoutPage({
  ...
  const subtotal = checkoutItems.reduce(
    (s, i) => s + i.product.minPrice * i.qty, 0
  )
  const couponDiscount = selectedCoupon?.discountAmount ?? 0
  const total = Math.max(0, subtotal - couponDiscount)
  const canOrder = selectedAddr !== null && payMethod

  function handleAddrAdd() {
    ...
    // TODO: API 연동 시 POST /api/addresses 호출
  }

  async function handleOrder() {
    if (!canOrder) return

    // 결제하기 클릭 시 각 상품마다 clickPurchaseButton 스니펫 전송
    ...
    // TODO: API 연동 시 POST /api/orders 호출
    onOrderComplete({
      items: checkoutItems,
      ...
      <div className="co-layout">
          ...
          <div className="co-card">
            <div className="co-card-header">
              ...
              {addresses.length > 0 && !showAddrForm && (
                <button
                  className="co-addr-change-btn"
                  onClick={() => { setShowAddrPicker(p => !p); setShowAddrForm(false) }}
                  type="button"
                >
              ...
              {selectedAddr && !showAddrPicker && !showAddrForm && (
                <div className="co-addr-selected">
                  {selectedAddr.isDefault && (
                    <span className="co-addr-default-badge">기본 배송지</span>
                  )}
                  <p className="co-addr-road">{selectedAddr.road}</p>
                  {selectedAddr.detail && (
                    <p className="co-addr-detail">{selectedAddr.detail}</p>
                  )}
                </div>
              )}
              ...
              {showAddrPicker && !showAddrForm && (
                <div className="co-addr-picker">
                  {addresses.map(addr => (
                    <div
                      key={addr.id}
                      className={`co-addr-option${selectedAddrId === addr.id ? ' selected' : ''}`}
                      onClick={() => { setSelectedAddrId(addr.id); setShowAddrPicker(false) }}
                    >
                      ...
                      <div className="co-addr-option-info">
                        ...
                        <p className="co-addr-road">{addr.road}</p>
                        {addr.detail && (
                          <p className="co-addr-detail">{addr.detail}</p>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <div className="co-addr-form">
                  ...
                  <div className="co-form-group">
                    ...
                    <input
                      ...
                      value={addrForm.road}
                      onChange={e => setAddrForm(p => ({ ...p, road: e.target.value }))}
                    />
                  </div>
                  <div className="co-form-group">
                    <label>상세 주소</label>
                  ...
                  <div className="co-addr-form-btns">
                    <button
                      className="co-addr-form-save"
                      onClick={handleAddrAdd}
                      type="button"
                      disabled={!addrForm.road.trim()}
                    >
                      추가하기
                    </button>
                    <button
                      className="co-addr-form-cancel"
                      onClick={() => { setShowAddrForm(false); setAddrForm(EMPTY_ADDR_FORM) }}
                      type="button"
                    >
                      취소
                    </button>
              ...
              {checkoutItems.map(({ product, qty }) => (
                <div key={product.productId} className="co-item">
                  ...
                  <span className="co-item-price">
                    {(product.minPrice * qty).toLocaleString()}원
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="co-card-body">
              <div className="co-pay-methods">
                {PAY_METHODS.map(m => (
                  <button
                    key={m.key}
                    className={`co-pay-btn ${m.className}${payMethod === m.key ? ' active' : ''}`}
        ...
        <div className="co-right">
          <div className="co-summary">
            ...
            <button className="co-back-btn" onClick={() => onNavigate('cart')}>
              장바구니로 돌아가기
            </button>
